...
| Rogue Legacy, STALKER: Call of Pripyat Mobile | Experimental | Rogue Legacy's clean reference and archived prior post-state recognize the jointly centered option rows, full 1320x990 map/teleporter target, and restored 720-line world-grid normalization; the upgraded APK reached signed post-state, installed on adopted storage, and cold-launched. STALKER upgrades settings text to 19/21/24/26 while accepting the earlier pass as input. Rogue Legacy visual confirmation remains pending. |
